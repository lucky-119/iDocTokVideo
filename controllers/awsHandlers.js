...
const meetingsTableName = process.env.MEETINGS_TABLE_NAME || "Meetings";
...
exports.end = async (event, context) => {
  ...
  const meeting = await getMeeting(event.title);

  // End the meeting. All attendee connections will hang up.
  await chime.deleteMeeting({ MeetingId: meeting.Meeting.MeetingId }).promise();
  console.log('end');
  await deleteMeetingFromDB(event.title);
  return response(200, 'application/json', JSON.stringify({}));
};
...
async function deleteMeetingFromDB(title) {
